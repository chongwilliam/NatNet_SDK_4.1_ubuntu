...
// global variables for skeleton link naming
std::vector<std::string> skeleton_link_names;

// global variables for velocity tracking
bool first_loop = true;
std::unordered_map<int, std::vector<Eigen::Vector3d>> prev_pos;
std::unordered_map<int, std::vector<Eigen::Vector4d>> prev_ori;
double prev_frame_timestamp = 0.0;

Eigen::Vector3d computeAngularVelocity(const Eigen::Quaterniond& q1, const Eigen::Quaterniond& q2, double dt) {
    // Compute the relative quaternion
    Eigen::Quaterniond dq = q2 * q1.conjugate();

    // Normalize the quaternion to avoid errors due to numerical imprecision
    dq.normalize();

    // Compute the angle (theta) of rotation
    double theta = 2.0 * std::acos(dq.w());

    // Avoid division by zero and check for small angle approximations
    double sin_half_theta = std::sqrt(1 - dq.w() * dq.w());
    if (sin_half_theta < 1e-6) {
        // Small angle approximation: theta is very small, return zero angular velocity
        return Eigen::Vector3d::Zero();
    }

    // Compute the axis of rotation
    Eigen::Vector3d axis(dq.x(), dq.y(), dq.z());
    axis /= sin_half_theta;

    // Compute angular velocity
    Eigen::Vector3d angular_velocity = (theta / dt) * axis;
...
std::string extractName(const std::string& str)
{
    ...
    size_t colons = str.find("::");

    if (underscore != std::string::npos) {
        size_t start = underscore + 1;

        if (colons != std::string::npos && colons > start)
            return str.substr(start, colons - start);

        return str.substr(start); // if no "::"
    }
...
void UpdateDataToDescriptionMaps(sDataDescriptions* pDataDefs)
{
    ...
    for (int i = 0; i < pDataDefs->nDataDescriptions; i++)
    {
        ...
        else if (pDataDefs->arrDataDescriptions[i].type == Descriptor_Skeleton)
        {
            ...
            {
                sRigidBodyDescription* pRB = &pSK->RigidBodies[j];
                skeleton_link_names.push_back(extractName(std::string(pRB->szName)));
                // skeleton_link_names.push_back(std::string(pRB->szName));
            }
...
void OutputFrameQueueToConsole()
{
    ...
    for (MocapFrameWrapper f : displayQueue)
    {
        ...
        if (!first_loop)
        {
            frameDeltaSeconds = data->fTimestamp - prev_frame_timestamp;
            if (frameDeltaSeconds <= 0.0)
            {
                frameDeltaSeconds = 1.0 / 100.0;
            }
        }
        ...
        for (i = 0; i < data->nSkeletons; i++)
        {
            ...
            for (int j = 0; j < skData.nRigidBodies; j++)
            {
                ...
                Eigen::Vector3d pos = Eigen::Vector3d(rbData.x, rbData.y, rbData.z);
                Eigen::VectorXd ori(4);
                ori << rbData.qx, rbData.qy, rbData.qz, rbData.qw;

                // set redis keys here
                std::string key_name = std::to_string(i) + "::" + std::to_string(j) + "::pos";
                redis_client->setEigenMatrixDerived(key_name, pos);

                key_name = std::to_string(i) + "::" + std::to_string(j) + "::ori";
                redis_client->setEigenMatrixDerived(key_name, ori);

                std::string key_name_with_string = skeleton_link_names[j] + "::pos";
                redis_client->setEigenMatrixDerived(key_name_with_string, pos);

                key_name_with_string = skeleton_link_names[j] + "::ori";
                redis_client->setEigenMatrixDerived(key_name_with_string, ori);

                // compute velocity through finite difference, and send 
                if (first_loop) {

                    key_name = std::to_string(i) + "::" + std::to_string(j) + "::lin_vel";
                    redis_client->setEigenMatrixDerived(key_name, Eigen::Vector3d::Zero());
                    prev_pos[i].push_back(Eigen::Vector3d::Zero());

                    key_name = std::to_string(i) + "::" + std::to_string(j) + "::ang_vel";
                    redis_client->setEigenMatrixDerived(key_name, Eigen::Vector4d::Zero());
                    prev_ori[i].push_back(Eigen::Vector4d::Zero());

                    key_name_with_string = skeleton_link_names[j] + "::lin_vel";
                    redis_client->setEigenMatrixDerived(key_name_with_string, Eigen::Vector3d::Zero());

                    key_name_with_string = skeleton_link_names[j] + "::ang_vel";
                    redis_client->setEigenMatrixDerived(key_name_with_string, Eigen::Vector3d::Zero());
                    
                } else {

                    Eigen::Vector3d lin_vel = (pos - prev_pos[i][j]) / frameDeltaSeconds;
                    key_name = std::to_string(i) + "::" + std::to_string(j) + "::lin_vel";
                    redis_client->setEigenMatrixDerived(key_name, lin_vel);
                    prev_pos[i][j] = pos;

                    Eigen::Quaterniond ori_prev(prev_ori[i][j](3), prev_ori[i][j](0), prev_ori[i][j](1), prev_ori[i][j](2));
                    Eigen::Quaterniond ori_curr(ori(3), ori(0), ori(1), ori(2));
                    Eigen::Vector3d ang_vel = computeAngularVelocity(ori_prev, ori_curr, frameDeltaSeconds);
                    key_name = std::to_string(i) + "::" + std::to_string(j) + "::ang_vel";
                    redis_client->setEigenMatrixDerived(key_name, ang_vel);
                    prev_ori[i][j] = ori;

                    key_name_with_string = skeleton_link_names[i] + "::lin_vel";
                    redis_client->setEigenMatrixDerived(key_name_with_string, lin_vel);

                    key_name_with_string = skeleton_link_names[i] + "::ang_vel";
                    redis_client->setEigenMatrixDerived(key_name_with_string, ang_vel);
        ...
        prev_frame_timestamp = data->fTimestamp;
